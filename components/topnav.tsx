    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <span className={cn('truncate text-sm font-medium', active ? 'text-white' : 'text-slate-200 group-hover:text-white')}>
            {item.label}
          </span>
          {item.badge ? (
    ...
    >
      <button
        ...
        className={cn(
          ...
          active
            ? cn('text-white', style.bg, 'border', style.border)
  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        className={cn(
          'flex items-center gap-2 rounded-xl border border-white/10 bg-slate-900/70 px-3 py-2 text-left transition hover:border-amber-500/20',
  ...
  return (
    <div ref={ref} className="relative">
      <button
        type="button"
        onClick={() => setOpen((c) => !c)}
        className="flex items-center gap-2 rounded-xl border border-white/10 bg-slate-900/70 px-2.5 py-1.5 text-left transition hover:border-amber-500/20"
      >
        <div className="relative flex h-7 w-7 items-center justify-center rounded-lg border border-white/10 bg-gradient-to-br from-slate-800 to-slate-700">
          <User className="h-3.5 w-3.5 text-amber-300" />
        </div>
        <div className="hidden min-w-0 max-w-[140px] lg:block">
          <p className="truncate text-xs font-semibold text-white">{displayName || 'Панель'}</p>
    ...
    <div className="fixed inset-0 z-[60] flex items-start justify-center bg-slate-950/70 p-6 pt-[10vh] backdrop-blur-sm">
      ...
      <div className="relative w-full max-w-xl overflow-hidden rounded-2xl border border-white/10 bg-slate-950/95 shadow-2xl">
        <div className="flex items-center gap-2 border-b border-white/5 px-4 py-3">
          ...
          />
          <kbd className="rounded-md border border-white/5 bg-slate-800 px-1.5 py-0.5 text-[10px] text-slate-400">ESC</kbd>
        </div>
...
export function TopNav() {
  ...
  return (
    <>
      <header className="sticky top-0 z-40 hidden border-b border-white/5 bg-slate-950/90 backdrop-blur-xl md:block">
        <div className="flex h-14 items-center gap-3 px-4 xl:px-6">
          <Link href="/dashboard" className="flex items-center gap-3">
            <AppLogoMark />
            <div className="hidden xl:block">
              <h1 className="bg-gradient-to-r from-white to-slate-300 bg-clip-text text-base font-bold leading-tight text-transparent">
                {SITE_NAME}
              </h1>
          ...
          <button
            ...
            className="flex items-center gap-2 rounded-xl border border-white/10 bg-slate-900/70 px-3 py-1.5 text-xs text-slate-400 transition hover:border-amber-500/20 hover:text-white"
          >
            <Search className="h-3.5 w-3.5" />
            <span className="hidden md:inline">Поиск</span>
            <span className="hidden items-center gap-1 rounded-md border border-white/5 bg-slate-700 px-1.5 py-0.5 text-[10px] text-slate-300 lg:flex">
