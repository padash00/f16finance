...
export function StoreShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-dvh bg-slate-950/30">
      ...
      <header className="sticky top-0 z-30 flex h-14 items-center justify-between gap-3 border-b border-white/10 bg-slate-950/80 px-3 backdrop-blur-xl sm:px-4">
        <div className="flex items-center gap-2">
          <button onClick={() => setMobileOpen(true)} className="grid h-9 w-9 place-items-center rounded-xl border border-white/10 bg-white/5 text-slate-300 hover:bg-white/10 md:hidden" aria-label="Меню">
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="hidden text-sm font-semibold text-emerald-300 sm:inline">Магазин</span>
              <span className="ml-1 hidden rounded-full border border-emerald-400/25 bg-emerald-500/10 px-2 py-0.5 text-[10px] font-medium uppercase tracking-wider text-emerald-300/90 lg:inline">модуль</span>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-1.5">
          <button onClick={() => setSearchOpen(true)} className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-slate-400 transition-colors hover:bg-white/10 hover:text-white">
            <Search className="h-3.5 w-3.5" />
            <span className="hidden sm:inline">Поиск</span>
            <span className="hidden items-center gap-0.5 rounded-md border border-white/10 bg-white/5 px-1.5 py-0.5 text-[10px] text-slate-400 lg:flex"><Command className="h-2.5 w-2.5" />K</span>
          </button>
          ...
          <button onClick={handleLogout} className="inline-flex items-center gap-1.5 rounded-xl border border-white/10 bg-white/5 px-2.5 py-1.5 text-xs font-medium text-slate-400 transition-colors hover:bg-rose-500/10 hover:text-rose-300" title="Выйти">
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <div className="absolute inset-y-0 left-0 flex w-[80%] max-w-[18rem] flex-col border-r border-white/10 bg-slate-950 shadow-2xl">
            <div className="flex h-14 items-center justify-between border-b border-white/10 px-4">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-semibold text-white">Магазин</span>
              </div>
              <button onClick={() => setMobileOpen(false)} className="grid h-8 w-8 place-items-center rounded-lg text-slate-400 hover:bg-white/5 hover:text-white"><X className="h-4 w-4" /></button>
            </div>
            <div className="flex-1 overflow-y-auto">{navList(() => setMobileOpen(false))}</div>
            <div className="border-t border-white/10 p-2">
              <Link href="/dashboard" onClick={() => setMobileOpen(false)} className="flex items-center gap-2.5 rounded-xl px-3 py-2.5 text-sm text-slate-400 hover:bg-white/5 hover:text-white">
                <ArrowLeft className="h-4 w-4" /> Назад в Orda
              </Link>
            </div>
          </div>
...
function StoreSearch({ onClose, navItems }: { onClose: () => void; navItems: Item[] }) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-start justify-center bg-slate-950/70 p-4 pt-[12vh] backdrop-blur-sm">
      ...
      <div className="relative w-full max-w-xl overflow-hidden rounded-2xl border border-white/10 bg-slate-950/95 shadow-2xl">
        <div className="flex items-center gap-2 border-b border-white/10 px-4 py-3">
          <Search className="h-4 w-4 text-slate-500" />
          <input ref={inputRef} value={q} onChange={(e) => setQ(e.target.value)} placeholder="Поиск по магазину: раздел, товар, штрихкод…" className="flex-1 bg-transparent text-sm text-slate-200 placeholder:text-slate-500 outline-none" />
          <kbd className="rounded-md border border-white/10 bg-white/5 px-1.5 py-0.5 text-[10px] text-slate-400">ESC</kbd>
        </div>
        <div className="max-h-[60vh] overflow-y-auto p-2">
          {navMatches.length > 0 && (
            <div className="mb-2">
              ...
              {navMatches.map((i) => {
                const Icon = i.icon
                return (
                  <button key={i.href} onClick={() => go(i.href)} className="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-left text-sm text-slate-200 hover:bg-white/5">
                    <span className="grid h-7 w-7 place-items-center rounded-lg bg-white/5 text-slate-400"><Icon className="h-4 w-4" /></span>
                    {i.label}
                  </button>
                )
              })}
          ...
          {q.trim() && (
            <div>
              ...
              {products.map((r, idx) => (
                <button key={`${r.href}-${idx}`} onClick={() => go(r.href)} className="flex w-full items-center justify-between gap-3 rounded-lg px-3 py-2 text-left text-sm text-slate-200 hover:bg-white/5">
