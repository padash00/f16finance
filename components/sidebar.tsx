...
const menuGroups = [
  ...
  },
  {
    ...
    items: [
      ...
      { icon: Shield, label: 'Debug', href: '/debug', special: true },
    ],
  },
]
  ...
  return (
    <div className="flex items-center gap-3 px-2">
      ...
      <div className="flex flex-col">
        ...
        <span className="text-[10px] text-gray-500">Управление клубом</span>
      </div>
    </div>
  )
}
...
function SidebarItem({ item, isActive, onClick }: { item: any; isActive: boolean; onClick?: () => void }) {
  ...
  return (
    ...
    >
      ...
      {/* Бейдж для чата */}
      {item.badge && (
        <span className="px-1.5 py-0.5 text-[8px] font-medium bg-emerald-500/20 text-emerald-400 rounded-full border border-emerald-500/30">
          {item.badge}
        </span>
      )}
