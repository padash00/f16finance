...
Скрипт скачивает официальный `LibreHardwareMonitor` 0.9.6, проверяет SHA-256 и включает только необходимые DLL. В архив не попадают исполняемый GUI и установщики драйверов. Распаковать `ORDA-Server-Monitor-1.1.2.zip` на сервер во временный каталог. Для дисков агент использует текущую `Composite Temperature`/`Temperature`; пороги `Warning Temperature` и `Critical Temperature` не считаются показаниями датчика.
