...
///
/// Заставка держится, пока приложение готовится, но не дольше пяти секунд.
/// Смысл не в красоте: за это время успевают восстановиться сессия и права, и
/// первый экран открывается заполненным, а не с «Проверяем доступ…» и пустыми
/// карточками. Готово раньше — уходит раньше; не готово за пять секунд — всё
/// равно уступает место, чтобы человек не смотрел в логотип.
struct LaunchAnimationView: View {
    /// Вызывается, когда заставка отработала.
    var onFinish: () -> Void
    /// Приложению больше нечего ждать.
    var isReady: () -> Bool = { true }

    /// Сколько держать знак после сборки, если готовность не наступила.
    private let maxHold: Duration = .milliseconds(3700)
    ...
    private func run() async {
        ...
        // Ждём готовности короткими шагами: так заставка уходит сразу, как
        // данные приехали, а не отстаивает положенное время впустую.
        var waited: Duration = .zero
        let step: Duration = .milliseconds(80)
        while !isReady(), waited < maxHold {
