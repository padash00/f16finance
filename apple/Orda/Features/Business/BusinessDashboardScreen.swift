...
struct BusinessDashboardScreen: View {
    ...
    var body: some View {
        ...
        // Сводка обновляется сама, пока открыта.
        //
        // Продажи идут не в телефоне: их пробивают на точке, в операторской
        // программе. Владелец держал экран открытым и видел цифры того
        // мгновения, когда его открыл, — чтобы узнать новое, надо было
        // догадаться потянуть вниз. Полминуты — компромисс: чаще незачем
        // (смена не меняется каждую секунду), реже уже читается как «висит».
        .task {
