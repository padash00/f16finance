...
struct OperatorRootView: View {
    ...
    var body: some View {
        ...
        .onChange(of: scenePhase) { _, phase in
            ...
            QuickActions.refreshForOperator(sellsGoods: sellsGoods)
            applyQuickAction()
            // Смену могли закрыть на кассе, пока телефон лежал в кармане.
            // Сверяем сразу, а не через круг опроса: иначе первые полминуты
            // человек смотрит на «смена открыта», которой уже нет.
            ...
            // Чек-листы и файлы, сделанные в подсобке: связь появляется,
            // когда человек возвращается к стойке и открывает приложение.
