...
public enum NativeSection: String, CaseIterable, Sendable {
    ...
    public var allowedStaffRoles: Set<String>? {
        ...
        //
        // Здесь были производство, модерация, план закупа, Telegram и оценка
        // бизнеса: их роуты смотрели на должность, и владелец, выдавший право
        // менеджеру, видел пункт меню, за которым его ждал отказ. Все роуты
        // переведены на права.
        //
        // Оценка бизнеса ушла последней. Её нельзя было просто открыть: права
        // работают «от разрешено», и снятая проверка роли показала бы оценку
        // компании всему штату. Теперь `valuation.view` выдаётся только явно —
        // по умолчанию его нет ни у кого, кроме владельца, и меню строится по
        // праву, как у всех остальных разделов.
        default: nil
        }
    }
    ...
    ///
    /// Сейчас таких раздвоений нет: оба роута спрашивают право своей страницы,
    /// правка выкачена. Механизм оставлен — расхождение меню и сервера
    /// заводится легко, а стоит человеку пустого экрана с отказом.
