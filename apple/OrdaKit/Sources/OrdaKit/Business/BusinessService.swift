...
public struct BusinessService: Sendable {
    ...
    /// пересчитали, а расхождения превращаются в движения и, если недостача,
    /// в долг. Приложение умело акт открыть и отменить, но не закрыть: ревизию
    /// проводили по полкам с телефоном, а завершали с ноутбука.
    ///
    /// Требует `store-revisions.commit`.
    public func closeAuditAct(id: String) async throws {
        _ = try await api.send(
            APIRequest(
                path: "/api/admin/store/audit",
                method: .post,
                body: try JSONSerialization.data(withJSONObject: ["action": "close", "act_id": id])
            )
        )
    }
