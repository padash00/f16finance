...
struct AccessResolverTests {
    ...
    func catalogIsPopulated() {
        ...
        // 86: плюс «Мониторинг сервера» — служебная страница суперадмина.
        #expect(summary.pages == 86)
        // Было 409. Убраны права, за которыми ничего не стояло: «Создать
        // перемещение», «Удалить запись долга», «Записать выплату», массовое
        // завершение и удаление задач, управление флагами устройства. Все они
        // либо дублировали право, которое сервер и проверяет, либо описывали
        // способ нажать кнопку, а не действие над данными.
        // Минус двойник «доп. рабочий день» (жил и в зарплате, и в
        // сотрудниках) и плюс «корректировка остатков витрины»: витрину и
        // склад правят разные люди, а право было одно на двоих.
        //
        // Минус пять выгрузок: файл собирает браузер из уже показанных цифр,
        // и право на него не решало ничего — кто видит таблицу, тот её и
        // выгрузит. Замок на картинке хуже отсутствующего: владелец снимал
        // право и считал, что закрыл выгрузку.
        //
        // Плюс два права мониторинга станций — «Мониторинг станций» и
        // «Подтверждение наблюдающих устройств»: на сайте они были, а в
        // каталоге приложения не хватало, и выгрузка их подобрала.
        //
        // Плюс пять прав «Мониторинга сервера» вместе с его страницей.
        #expect(summary.capabilities == 405)
    }
