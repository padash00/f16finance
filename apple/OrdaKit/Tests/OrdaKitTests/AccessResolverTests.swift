...
struct AccessResolverTests {
    ...
    func catalogIsPopulated() {
        ...
        // Было 409. Убрано три права, за которыми ничего не стояло:
        // «Создать перемещение» (маршрута нет, перемещение идёт под правом
        // переноса на витрину), «Удалить запись долга» и «Записать выплату» —
        // последние два дублировали права, которые сервер и проверяет.
        #expect(summary.capabilities == 406)
    }
