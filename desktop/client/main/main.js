...
const SERVER_WS_URL = process.env.KIOSK_WS_URL || 'ws://127.0.0.1:8787/ws/client'
...
function onTick() {
  ...
  if (session.active && remaining === 300) {
    pushState()
  } else {
    pushState()
  }
...
function setupIpc() {
  ipcMain.handle('kiosk:launch-game', async (_event, gameId) => {
    ...
    try {
      const result = launchConfiguredGame(gameId, DEFAULT_GAME_PATH)
      sendStatus('in_game')
      pushState()
      return result
    } catch (error) {
...
app.on('window-all-closed', () => {
  // keep running kiosk; do not quit
})
