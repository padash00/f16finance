...
def parse_money(raw: str) -> int:
    ...
    try:
        # Удаляем пробелы, запятые и другие разделители
...
class DebtTab(QWidget):
    ...
    def load_debts(self):
        ...
        try:
            ...
            self.info_label.setText(f"⚠️ Сервер недоступен: {error}")
    ...
    def update_table(self):
        ...
        for row_index, item in enumerate(items_to_show):
            ...
            debtor_item = QTableWidgetItem(f"👤 {debtor_name}")
            self.table.setItem(row_index, 1, debtor_item)
            
            # Товар
            item_name = str(item.get("item_name") or "")
            item_item = QTableWidgetItem(f"📦 {item_name}")
