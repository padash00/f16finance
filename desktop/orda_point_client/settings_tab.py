...
class ValidatedField(QFrame):
    ...
    def set_state(self, state: ValidationState, message: str = ""):
        ...
        if state == ValidationState.VALID:
            ...
            self.state_icon.setStyleSheet(f"color: {SUCCESS};")
        elif state == ValidationState.INVALID:
            self.state_icon.setText("❌")
            self.state_icon.setStyleSheet(f"color: {DANGER};")
        elif state == ValidationState.WARNING:
            self.state_icon.setText("⚠️")
            self.state_icon.setStyleSheet(f"color: {WARNING};")
        else:
            self.state_icon.setText("")
...
class SettingsTab(QWidget):
    ...
    def setup_advanced_section(self):
        ...
        autosave_label.setStyleSheet(f"color: {TEXT}; font-size: 13px;")
