...
export default function ArenaPage({
  ...
  async function handleStart(tariffId: string, payMethod: 'cash' | 'kaspi' | 'mixed', cashAmt: number, kaspiAmt: number, discPct: number) {
    ...
    try {
      ...
      const usedTariff = tariffs.find(t => t.id === tariffId)
      const startMs = new Date(newSession.started_at).getTime()
      const endMs = new Date(newSession.ends_at).getTime()
      const durationMin = Math.round((endMs - startMs) / 60000)
      setPrintReceipt({
        stationName: startTarget.name,
        tariffName: usedTariff?.name ?? 'Тариф',
        durationMin,
        startedAt: newSession.started_at,
        endsAt: newSession.ends_at,
        amount: newSession.amount,
        cashAmount: newSession.cash_amount,
        kaspiAmount: newSession.kaspi_amount,
        paymentMethod: newSession.payment_method,
        operatorName: session.operator.short_name || session.operator.name || session.operator.username,
        clubName: session.company.name,
      })
      setStartTarget(null)
  ...
  async function handleEndWithRefund() {
    if (!manageTarget) return
    setActionLoading(true)
    try {
      const { refund } = await api.endArenaSessionWithRefund(config, session, manageTarget.session.id)
      setSessions((prev) => prev.filter((s) => s.id !== manageTarget.session.id))
      setManageTarget(null)
      if (refund.amount > 0) {
      ...
      toastError(err?.message || 'Не удалось завершить сессию с возвратом')
    } finally {
      setActionLoading(false)
    }
  }
  ...
  function handleExportCsv() {
    const rows = sessions.map((s) => {
      const st = stations.find(x => x.id === s.station_id)
      const t = tariffs.find(x => x.id === s.tariff_id)
      const startMs = new Date(s.started_at).getTime()
      const endMs = new Date(s.ends_at).getTime()
      const durationMin = Math.round((endMs - startMs) / 60000)
      return [
        st?.name ?? s.station_id,
        t?.name ?? s.tariff_id ?? '',
        durationMin,
        new Date(s.started_at).toLocaleString('ru-RU'),
        new Date(s.ends_at).toLocaleString('ru-RU'),
        s.amount,
        s.cash_amount,
        s.kaspi_amount,
        s.payment_method,
    ...
    a.href = url
    a.download = `arena-sessions-${new Date().toISOString().slice(0, 10)}.csv`
    a.click()
    ...
    <div className="relative flex h-screen flex-col bg-background text-foreground">
      ...
      {manageTarget && !techTarget && (
        <ManageSessionModal
          ...
          onEnd={handleEnd}
          onEndWithRefund={handleEndWithRefund}
      ...
      {/* Print receipt modal */}
      {printReceipt && (
        <PrintReceiptModal receipt={printReceipt} onClose={() => setPrintReceipt(null)} />
      )}
...
function formatReceiptTime(iso: string) {
  return new Date(iso).toLocaleString('ru-RU', { hour: '2-digit', minute: '2-digit', day: '2-digit', month: '2-digit', year: 'numeric' })
}

function formatReceiptMoney(n: number) {
  return n.toLocaleString('ru-RU') + ' ₸'
}
...
<title>Чек</title>
<style>
  body { font-family: 'Courier New', monospace; font-size: 13px; margin: 0; padding: 12px; width: 280px; }
...
<body>
<h1>${receipt.clubName}</h1>
<p class="sub">Оператор: ${receipt.operatorName}</p>
<hr/>
<div class="row"><span class="label">Станция:</span><span>${receipt.stationName}</span></div>
<div class="row"><span class="label">Тариф:</span><span>${receipt.tariffName}</span></div>
<div class="row"><span class="label">Длительность:</span><span>${receipt.durationMin} мин</span></div>
<hr/>
<div class="row"><span class="label">Начало:</span><span>${formatReceiptTime(receipt.startedAt)}</span></div>
<div class="row"><span class="label">Конец:</span><span>${formatReceiptTime(receipt.endsAt)}</span></div>
<hr/>
<div class="row total"><span>Итого:</span><span>${formatReceiptMoney(receipt.amount)}</span></div>
<div class="row"><span class="label">Оплата:</span><span>${payStr}</span></div>
<p class="footer">Спасибо за визит!</p>
</body>
</html>`
...
function SessionHistoryModal({
  ...
  const [rows, setRows] = useState<ArenaSession[]>([])
  ...
  async function load() {
    setLoading(true)
    try {
      const data = await api.getArenaSessions(config, session, from, to)
      setRows(data)
      setLoaded(true)
    } catch (err: any) {
      toastError(err?.message || 'Не удалось загрузить историю')
    } finally {
      setLoading(false)
    }
  }
  ...
  function exportCsv() {
    ...
    const csvRows = rows.map((r) => {
      ...
      const stName = st?.name ?? (r as any)['arena_stations']?.name ?? r.station_id
      ...
      return [
        ...
        r.status, r.amount, r.cash_amount, r.kaspi_amount, r.payment_method,
      ]
    })
    ...
    const a = document.createElement('a')
    a.href = url
    a.download = `history-${from}-${to}.csv`
    a.click()
    URL.revokeObjectURL(url)
  }
  ...
  const totalKaspi = rows.reduce((s, r) => s + (r.kaspi_amount || 0), 0)
      ...
      <div className="flex w-full max-w-3xl flex-col rounded-2xl border border-border bg-background shadow-2xl" style={{ maxHeight: '85vh' }}>
        ...
        <div className="flex shrink-0 items-center gap-3 border-b px-6 py-3">
          ...
          <button
            ...
            onClick={load}
            disabled={loading}
            className="rounded-lg bg-primary px-4 py-1.5 text-sm font-medium text-primary-foreground transition hover:opacity-90 disabled:opacity-50"
          >
