...
### Knowledge Admin: чек-листы приведены к стилю статей

Сделано:
- Вкладка `/knowledge-admin` -> `Чек-листы` переделана в тот же UX-паттерн, что и `Статьи и FAQ`: один широкий рабочий список, верхняя зона с поиском, фильтрами и кнопками создания.
- Убран отдельный правый сайдбар с быстрыми сценариями и формами, из-за которого вкладка визуально и логически отличалась от статей.
- Быстрые шаблоны чек-листов перенесены в компактные chip-кнопки рядом с фильтрами.
- Создание и редактирование чек-листа/пункта теперь раскрывается inline только по действию пользователя.
- Карточки чек-листов визуально выровнены со статьями: badges, заголовок, описание, статистика, действия справа, пункты внутри карточки.
...
Ожидаемый результат:
- Вкладки `Статьи и FAQ` и `Чек-листы` больше не ощущаются как две разные страницы.
- Администратор сначала видит список и фильтры, а формы появляются только когда нужно создать или изменить сценарий.
...
### 2026-04-26 — умный редактор чек-листов в базе знаний

Сделано:
- Вкладка `Чек-листы` на `/knowledge-admin` получила полноценный модальный редактор в стиле редактора статей: настройки сценария, пункты чек-листа и предпросмотр для оператора.
- Добавлены быстрые настройки сценария: точка, роль, смена, тип запуска, интервал обхода, блокировка смены, активность и порядок.
- Добавлен редактор пунктов: привязка к сценарию, категория FAQ, статья-подсказка, тип ответа, важность, штраф, бонус, обязательность и требование фото.
- Добавлен предпросмотр, чтобы сразу видеть, как чек-лист будет выглядеть для оператора до сохранения.
- Старые встроенные формы чек-листов убраны со страницы, чтобы вкладка выглядела так же чисто и понятно, как `Статьи и FAQ`.

Проверка:
- `npx eslint "components/admin/checklist-editor-dialog.tsx" "app/(main)/knowledge-admin/page.tsx"` ✅

### 2026-04-26 — исправлены инструменты Word-редактора статей

Сделано:
- В `components/admin/rich-text-editor.tsx` тулбар TipTap перестал забирать фокус у редактора, поэтому форматирование теперь применяется к выделенному тексту, как в обычном Word/WYSIWYG.
- Заменён проблемный color input на быстрые цветовые кнопки, которые не сбрасывают выделение.
- Добавлены быстрые маркеры/подсветка и сброс подсветки.
- Добавлены явные стили для контента редактора: H1/H2/H3, списки, цитата, код, ссылки, картинки, таблицы, жирный/курсив/подчёркивание/зачёркивание.
- Убрана зависимость визуального результата от `prose`/typography-стилей, чтобы пользователь сразу видел, что кнопка реально сработала.
...
### 2026-04-26 — умная вкладка чек-листов в базе знаний

Сделано:
- В `/knowledge-admin` вкладка `Чек-листы` переделана из простого списка в полноценный конструктор сценариев для операторов.
- Добавлена сводка по чек-листам: активные сценарии, блокирующие смену, пункты с фото и пункты с рисками/штрафами.
- Добавлены поиск, фильтр по точке и фильтр по типу сценария: открытие, обход, закрытие, онбординг и передача.
- Карточки чек-листов теперь показывают понятные русские статусы роли, смены, периодичности, блокировки, активности и статистику пунктов.
- Внутри карточек пункты чек-листа показывают важность, тип ответа, обязательность, фото, штраф/бонус и привязанную FAQ-статью.
- Справа добавлены быстрые пресеты сценариев: открытие смены, обход зала, закрытие смены и онбординг нового оператора.
- Формы создания шаблона и пункта сделаны разборчивее: понятные подписи, пояснения, категория, FAQ-связь, важность, штраф/бонус, активность и порядок.

Проверка:
- `npm run typecheck` ✅
- `npm run lint` ✅ без новых ошибок, остались только существующие предупреждения проекта

### 2026-04-24 — расширено управление проектами точек

Сделано:
- На странице `/point-devices` форма создания/редактирования проекта стала понятнее: выбранные точки показываются сверху, свободные ниже, у каждой есть явная кнопка `Добавить` или `Убрать`.
- В проектные настройки добавлены все рабочие флаги операторки: сменные отчёты, доходы, долги/сканер, суточная сверка Kaspi, старт кассы, арена/станции, автоитоги арены и режим без авто-доходов.
- Для каждой отдельной точки внутри проекта добавлено управление исключениями: `Наследовать от проекта`, `Включить`, `Выключить`.
- Исправлено сохранение per-point настроек: теперь точка сохраняет только явно выбранные исключения и не сбрасывает остальные флаги проекта в `false`.
- API `/api/admin/point-devices` теперь отдельно нормализует настройки проекта и частичные настройки конкретных точек.
...
### 2026-04-28 — операторские чек-листы и журнал прохождений

Сделано:
- В операторскую desktop-программу добавлен отдельный экран прохождения чек-листов смены.
- На экране смены появилась быстрая кнопка `Чек-листы смены`, чтобы оператор мог открыть обязательные сценарии без админки.
- В кабинете оператора появилась вкладка `Правила и FAQ`: оператор видит материалы базы знаний, обязательные подтверждения и может нажать `Ознакомлен`.
- Подтверждение материалов теперь работает и от `operator_id`: API сам находит связанный `staff_id` через `operator_staff_links`.
- `/api/point/knowledge` теперь отдаёт оператору не только статьи/FAQ, но и активные шаблоны чек-листов, пункты, текущую открытую смену и уже начатые прохождения.
- `/api/point/checklist/run` умеет стартовать прохождение чек-листа от текущего оператора и связывать его со staff через `operator_staff_links`.
- `/api/point/checklist/run/[id]/complete` сохраняет ответы, считает штрафы/бонусы, проставляет `run_by`, закрывает прохождение и создаёт инциденты по проваленным/бонусным пунктам.
- Чек-листы с режимом `onboarding` и `periodic` теперь подсвечиваются оператору как требующие прохождения, если их ещё не проходили или прошло заданное количество минут.
- При закрытии смены добавлена мягкая блокировка: если у открытой смены есть обязательные `blocks_shift` чек-листы без завершения, оператор получает понятное сообщение и переходит на чек-листы.
- В `/knowledge-admin` добавлена вкладка `Журнал чек-листов`: видно историю прохождений, статус, точку, оператора, прогресс ответов, штрафы, бонусы и проблемные пункты.
- В журнале прохождений теперь можно раскрыть детали ответов по каждому пункту: ответ, комментарий, обязательность, фото-требование, штраф и бонус.
- Исправлены мелкие typecheck-ошибки в кнопках обновления склада, где обработчик клика случайно совпадал с функцией, принимающей `AbortSignal`.

Проверка:
- `npm run typecheck` ✅
- `npm run lint -- --quiet` ✅
- `npm run build` ✅
- `npm run build` в `desktop/operator` ✅
