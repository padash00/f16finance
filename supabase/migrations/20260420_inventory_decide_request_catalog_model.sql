-- Catalog-модель: одобрение заявки только уменьшает источник (склад),
-- виртуальная витрина = catalog - warehouse, поэтому target инкрементить не нужно.
-- target_location_id остаётся в таблице/аргументах ради обратной совместимости
-- (точки, кассы, operator) — старые записи валидны, но новая логика её не меняет.
...
begin
  ...
  loop
    ...
    if v_approved_qty > 0 then
      ...
      -- Движение: from = склад, to оставляем как было (для аудита), но по остаткам
      -- никуда ничего не поступает — это аллокация в витрину, которая вычисляемая.
