-- Разрешить менеджеру одобрять БОЛЬШЕ, чем запросил оператор.
-- Реальный сценарий: оператор ошибся в количестве (указал 5 вместо 10),
-- менеджер при одобрении правит "вверх" и сохраняет. Раньше БД-функция
-- выбрасывала 'inventory-request-approved-qty-too-high', теперь это разрешено.
-- Отрицательные значения по-прежнему блокируются.
...
begin
  ...
  loop
    ...
    if v_approved_qty > 0 then
      -- Catalog-модель: уменьшаем только склад. Витрина = catalog - warehouse (виртуально).
  ...
  -- Статус: "full" если все позиции одобрены не меньше запрошенного,
  -- "partial" если есть хоть одна с меньшим (или обнулённая при ненулевом запросе).
  if exists (
    select 1
    from public.inventory_request_items iri
    where iri.request_id = p_request_id
