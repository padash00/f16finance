-- Этап 1 фичи «поставщики»: торговый представитель + закрепление товара.
--
--  - inventory_suppliers: торгпред (имя + WhatsApp-телефон) и срок поставки
--    (lead_time_days — нужен для умного порога на Этапе 2).
--  - inventory_items.primary_supplier_id: «закреплённый» поставщик. Ставится
--    автоматически при приёмке (последний поставщик становится основным),
--    можно переопределить вручную.

alter table public.inventory_suppliers
  add column if not exists sales_rep_name text null,
  add column if not exists sales_rep_phone text null,
  add column if not exists lead_time_days integer not null default 3;

alter table public.inventory_items
  add column if not exists primary_supplier_id uuid null
    references public.inventory_suppliers(id) on delete set null;

create index if not exists inventory_items_primary_supplier_idx
  on public.inventory_items (primary_supplier_id)
  where primary_supplier_id is not null;

-- Бэкфилл: закрепляем поставщика по последней приёмке каждого товара.
update public.inventory_items it
set primary_supplier_id = sub.supplier_id
from (
  select distinct on (ri.item_id) ri.item_id, r.supplier_id
  from public.inventory_receipt_items ri
  join public.inventory_receipts r on r.id = ri.receipt_id
  where r.supplier_id is not null
  order by ri.item_id, r.received_at desc, r.created_at desc
) sub
where it.id = sub.item_id
  and it.primary_supplier_id is null;

-- Обновляем RPC приёмки: при проведении закрепляем поставщика за каждым товаром.
