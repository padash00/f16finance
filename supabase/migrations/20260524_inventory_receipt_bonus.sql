-- Бонусный товар в приёмке.
-- Поставщики иногда дают товар в подарок: в накладной только количество,
-- цены нет. Помечаем такие строки флагом is_bonus:
--   - себестоимость = 0, в сумму накладной не входит,
--   - остаток и движение создаются как обычно (товар физически пришёл),
--   - при продаже вся выручка идёт в прибыль (мы за товар не платили).

alter table public.inventory_receipt_items
  add column if not exists is_bonus boolean not null default false;

-- Обновляем RPC: тот же интерфейс, но обрабатываем is_bonus в строках.
...
begin
  ...
  loop
    ...
    -- Бонус: себестоимость принудительно 0, в сумму накладной не идёт.
