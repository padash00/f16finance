...
export type ShiftFact = {
  company_id: string
  /**
   * Объект смены (`point_shifts.id`), если продажи к нему привязаны. По нему
   * открываются чеки и позиции смены на странице «Смены».
   * null — историческая смена без привязки.
   */
  shift_id?: string | null
  /**
   * Сколько минут работала смена. Смена на шесть часов вместо двенадцати даёт
   * вдвое меньше покупателей — без длительности это читалось бы как падение
  ...
  /**
   * Множитель цен месяца этой смены относительно базового.
   *
   * Нужен, чтобы повышение цен не выглядело улучшением работы продавца:
   * денежные метрики сравниваются в ценах базы. 1 — поправки нет.
   */
  price_index?: number
}
