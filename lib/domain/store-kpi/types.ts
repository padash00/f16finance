...
export type ShiftFact = {
  ...
  /**
   * Результат последнего сданного экзамена продавца, 0..1.
   *
   * Заполняется, только если экзамен свежий: знание, подтверждённое полгода
   * назад, о сегодняшней работе не говорит. null — не проверяли, и это НЕ
   * ноль: ноль означал бы «знает на ноль», а на деле мы просто не знаем.
   */
   ...
   *
   * Считается ВНУТРИ одного чека и по факту присутствия, а не по количеству.
   * Два рамена и один напиток в чеке — одна возможность и одна засчитанная
   * допродажа, а не «одна из двух». Решение владельца, 2026-08-16: продавец
   * предложил напиток к заказу один раз, и требовать по напитку на каждую
   * порцию значит наказывать за то, чего покупатель не просил.
   *
   * Рамен одним чеком и напиток следующим — это НЕ допродажа: две отдельные
   * покупки. Так и должно быть, менять нельзя.
   */
  attach_opportunities: number
  /** Сколько из них закрыто — второй товар оказался в том же чеке. */
