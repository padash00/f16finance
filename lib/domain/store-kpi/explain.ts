...
function readMetric(m: MetricRatio): string {
  ...
  switch (m.metric) {
    ...
    case 'attach_rate':
      return `Допродажи ${direction}. Считается внутри одного чека по вашим правилам «взяли одно — предложи другое»: рамен и напиток в одном чеке — допродажа, разными чеками — нет.`
