export const PUBLIC_PATHS = [
  ...
  '/point-terminal',
  '/offer',
  // Инструкция открыта всем: её читают до покупки и присылают новому
  // сотруднику, у которого учётной записи ещё нет. Подстраницы (регламент
  // оператора) — тоже: matchesPath требует явного «/*».
  '/help',
  '/help/*',
  '/privacy',
  '/terms',
...
export const ACCESS_PAGE_GROUPS: readonly AccessPageGroup[] = [
  ...
  },
  {
    ...
    pages: [
      ...
      { path: '/expenses/new', label: 'Добавить расход' },
      { path: '/expenses/pending', label: 'Ожидают одобрения расходов' },
      { path: '/expense-whitelist', label: 'Доверенные поставщики расходов' },
