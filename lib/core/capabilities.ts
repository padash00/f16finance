...
/**
 * Права, которые не выдаются сами собой.
 *
 * Обычное право работает «от разрешено»: сотрудник получает весь каталог, а
 * владелец на /access что-то отнимает. Для большинства разделов это верно —
 * человека нанимают работать, а не выпрашивать доступы.
 *
 * Но есть данные, которые открывают не по умолчанию, а по решению: оценка
 * стоимости бизнеса — это то, за сколько компанию можно продать. Такое право
 * должно появляться у человека только тогда, когда владелец его назвал вслух.
 *
 * Владелец организации и супер-админ проходят как обычно: у них полный набор.
 */
export const DENY_BY_DEFAULT_CAPABILITIES = new Set<string>([
  'valuation.view',
])

/** true, если право выдаётся только явно — само по себе не появится. */
export function isDenyByDefault(capability: string): boolean {
  return DENY_BY_DEFAULT_CAPABILITIES.has(capability)
}
