...
export const takeTaskTool: CopilotTool = {
  ...
  handler: async (input, ctx) => {
    ...
    const { data: task, error: getErr } = await ctx.supabase.from('tasks').select('id, title, status, company_id').eq('id', taskId).single()
    if (getErr || !task) return { ok: false, message: 'Задача не найдена.' }

    // Мультитенантная изоляция: брать в работу можно только задачу своей организации.
