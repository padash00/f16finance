...
export const navSections: NavSection[] = [
  {
    ...
    items: [
      ...
      { href: '/ai-cfo', label: 'AI Финдиректор', icon: Briefcase, note: 'Почему прибыль такая и что с этим делать', badge: 'AI', badgeColor: 'purple', isNew: true, feature: 'ai.cfo' },
  ...
  },
  {
    ...
    items: [
      ...
      { href: '/store/shifts', label: 'Смены магазина', icon: Clock, note: 'Сменные отчёты кассы: выручка, чеки, Z-отчёт' },
      { href: '/store/insights', label: 'AI-разбор', icon: Sparkles, note: 'Что говорят цифры магазина и что с этим делать', badge: 'AI', badgeColor: 'purple' },
      { href: '/store/receipt-settings', label: 'Реквизиты чека ККМ', icon: Receipt, note: 'Налогоплательщик, ККМ, ОФД, НДС — приказ МФ РК №626' },
      { href: '/store/settings', label: 'Настройки магазина', icon: SettingsIcon, note: 'Какие точки работают как магазин' },
    ],
  },
  {
    id: 'team',
...
export const BASE_FREE_PATHS = new Set<string>([
  ...
  '/workspace',
  '/pass',
  // Токены/подключение кассовых программ — нужно ЛЮБОМУ клиенту независимо от
  // пакета, иначе владелец не сможет подключить операторскую программу.
  '/point-devices',
])
