...
function detailPages(d, startPageNo, totalPages) {
  ...
  // flatten в линии: групп-заголовок + строки
  const lines = []
    ...
    const rowsHtml = lns.map((ln) => {
      if (ln.kind === 'group') {
        ...
        return `<tr class="dt-group">
          ...
          <td class="g-meta" colspan="${Math.max(1, last - 1)}">${esc(g.meta || '')}</td>
          <td class="num g-total" colspan="2">${esc(totalTxt)}</td>
        </tr>`
      }
      const r = ln.r
      return `<tr>${cols.map((c) => `<td class="${c.align === 'right' ? 'num' : ''}">${cell(r[c.key], c.align)}</td>`).join('')}</tr>`
...
export function renderPremiumHTML(d, { fontCss = '' } = {}) {
  ...
  const hasDetail = d.detail && (d.detail.groups || []).length > 0
