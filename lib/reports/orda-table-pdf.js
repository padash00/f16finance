...
function row(cols, r) {
  return `<tr>${cols.map((c) => `<td class="${c.align === 'right' ? 'num' : ''}">${fmtCell(r[c.key])}</td>`).join('')}</tr>`;
}
...
export function renderTableHTML(d, { fontCss = '' } = {}) {
  ...
  const totalRow = d.total
    ? `<tr class="total">${cols.map((c, i) => `<td class="${c.align === 'right' ? 'num' : ''}">${i === 0 && d.total[c.key] == null ? 'ИТОГО' : fmtCell(d.total[c.key])}</td>`).join('')}</tr>`
    : '';

  const pages = chunks.map((chunk, ci) => {
    const isLast = ci === chunks.length - 1;
    const body = chunk.map((r) => row(cols, r)).join('') || `<tr><td colspan="${cols.length}" class="empty">Нет данных за период</td></tr>`;
    return `<div class="page">${header(d.meta, ci + 1, total)}<div class="body">
      <div class="card grow">
        <div class="card-h">${esc(d.meta.title || 'Данные')} <span class="muted">${rows.length} строк${chunks.length > 1 ? ` · часть ${ci + 1}/${chunks.length}` : ''}</span></div>
        <table class="tbl">${thead}<tbody>${body}${isLast ? totalRow : ''}</tbody></table>
        ${isLast && d.note ? `<div class="note">${esc(d.note)}</div>` : ''}
      </div>
