...
// Бюджет высоты tbody ≈ 582px (A4 landscape − шапка/футер/картон/thead).
// Обычная строка ≈ 23px → максимум 25.3; берём 24 с запасом (26 вылезало за рамку).
// Dense-строка ≈ 19.4px → максимум ~30; берём 28.
const ROWS_PER_PAGE = 24;
const ROWS_PER_PAGE_DENSE = 28;
...
export function renderTableHTML(d, { fontCss = '' } = {}) {
  ...
  for (const s of sections) {
    const rows = s.rows || [];
    const perPage = s.dense ? ROWS_PER_PAGE_DENSE : ROWS_PER_PAGE;
    const chunks = [];
...
table.tbl td{font-size:10px;line-height:1.2;padding:5px 7px;border-bottom:1px solid #f4f7fa;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
