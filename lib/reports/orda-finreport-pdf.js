...
function overviewPage(d, pageNo, total) {
  ...
  return `<div class="page">${header(d.meta, pageNo, total, 'Обзор')}<div class="body">
    <div class="kpis">
      ...
      <div class="kpi"><div class="kpi-l">ВЫРУЧКА НА ЗАПИСЬ</div><div class="kpi-v">${fmt(k.avgCheck)} ₸</div><div class="kpi-s">${k.txns} записей дохода</div></div>
    </div>
    <div class="grid2">
      <div class="col">
        <div class="card">
...
function companiesPage(d, pageNo, total) {
  ...
  const cards = comp.map((c) => {
    ...
    return `<div class="cc">
      ...
      <div class="cc-kv"><div><span>Записей дохода</span><b>${fmt(c.txns)}</b></div><div><span>Выручка на запись</span><b>${fmt(avg)} ₸</b></div></div>
    </div>`;
