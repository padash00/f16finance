...
export async function POST(req: Request) {
  try {
    ...
    // Telegram chat ID — только целое число (положительное для пользователей, отрицательное для групп)
    if (chatIdRaw !== null && !/^-?\d+$/.test(chatIdRaw)) {
      return NextResponse.json({ error: 'Неверный формат telegram_chat_id' }, { status: 400 })
    }
