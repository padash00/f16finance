...
export async function POST(request: Request) {
  try {
    ...
    if ('error' in result) {
      ...
      // Как система прочитала ввод — не тайна: это следует из самой строки,
      // которую человек напечатал. А знать это важно: операторы входят по
      // логину, сотрудники по почте, и решает наличие «@». Оператор, набравший
      // свою почту, попадал в контур сотрудников и получал «неверный логин или
      // пароль» — без единой зацепки, что не так. В журнале такие попытки идут
      // парами по две-три секунды: человек пробует то же самое ещё раз.
      //
      // Существует ли учётная запись, по-прежнему не сообщаем: подсказка
      // говорит только о разборе ввода.
      const hint = isOperatorLogin
        ? 'Ввод прочитан как логин оператора. Если вы сотрудник — войдите по рабочей почте.'
        : 'Ввод прочитан как почта сотрудника. Если вы оператор — введите логин, без почты.'

      return json(
        {
          error: result.error,
          login_type: isOperatorLogin ? 'operator' : 'staff',
          message:
            result.error === 'invalid-credentials'
              ? `Неверный логин или пароль. ${hint}`
