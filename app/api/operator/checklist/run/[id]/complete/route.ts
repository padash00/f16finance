...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: items } = await supabase
    ...
    .select(
      'id, fine_amount, bonus_amount, severity, is_required, requires_photo, answer_type, title, knowledge_article_id',
    )
  ...
  if (targetStatus === 'completed') {
    const missingRequired = itemsArr.filter((item) => {
      ...
      if (item.answer_type === 'boolean') {
        const hasBooleanAnswer =
          typeof response.passed === 'boolean' || typeof response.value === 'boolean'
  ...
  const resolvedStaffId = (run as any).run_by || staffId || null
  ...
  // Авто-инциденты: для каждого item с штрафом/бонусом
  if (targetStatus !== 'skipped') {
    ...
    for (const item of itemsArr) {
      ...
      const photoUrls =
        typeof r?.photo_data_url === 'string' && r.photo_data_url.trim()
          ? [r.photo_data_url]
          : []
    ...
    for (const inc of incidents) {
      const { error: incError } = await supabase.rpc('incidents_create', {
        ...
        p_subject_staff_id: resolvedStaffId,
  ...
  return json({ run_id: id, status: targetStatus, fines_total: finesTotal, bonuses_total: bonusesTotal })
}
