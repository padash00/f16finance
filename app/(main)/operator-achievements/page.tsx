...
function AchievementCard({
  ...
  return (
    <div
      className={`rounded-xl border ${earned ? c.border : 'border-white/8'} ${
        earned ? c.bg : 'bg-white/[0.02]'
      } p-4 ${!earned ? 'opacity-70' : ''}`}
    >
      <div className="flex items-start gap-3">
        <div
          className={`grid h-10 w-10 shrink-0 place-items-center rounded-lg ${
            earned ? c.bg : 'bg-white/5'
          } ${earned ? c.text : 'text-slate-500'}`}
        >
          <AchievementIconCmp kind={ach.icon} className="w-5 h-5" />
        </div>
        <div className="min-w-0 flex-1">
          <div className={`text-sm font-semibold ${earned ? 'text-white' : 'text-slate-300'}`}>
            {ach.title}
          </div>
