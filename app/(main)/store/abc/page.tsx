...
const ABC_COLORS: Record<AbcClass, string> = {
  A: 'border-emerald-500/30 bg-emerald-500/10 text-emerald-700 dark:text-emerald-200',
  B: 'border-amber-500/30 bg-amber-500/10 text-amber-700 dark:text-amber-200',
  C: 'border-rose-500/30 bg-rose-500/10 text-rose-700 dark:text-rose-200',
}
    ...
    <div className={embedded ? 'space-y-6' : 'app-page-wide space-y-6'}>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <div className="grid grid-cols-2 gap-1 rounded-xl border border-border bg-slate-100 dark:bg-white/[0.03] p-0.5">
              <button
                ...
                className={`rounded-lg px-3 py-1.5 text-sm transition ${tab === 'sales' ? 'bg-white dark:bg-white/10 text-foreground' : 'text-muted-foreground hover:text-foreground'}`}
              >
                По продажам
              </button>
              <button
                ...
                className={`rounded-lg px-3 py-1.5 text-sm transition ${tab === 'stock' ? 'bg-white dark:bg-white/10 text-foreground' : 'text-muted-foreground hover:text-foreground'}`}
              >
                По запасам
              </button>
            </div>
              ...
              <div className="inline-flex rounded-xl border border-border bg-slate-100 dark:bg-white/[0.03] p-0.5">
                {PERIODS.map((p) => (
                  <button
                    ...
                    className={`rounded-lg px-3 py-1.5 text-xs transition ${period === p.value ? 'bg-white dark:bg-white/10 text-foreground' : 'text-muted-foreground hover:text-foreground'}`}
                  >
                    {p.label}
                  </button>
                ))}
      ...
      {totalForBar > 0 ? (
        <Card className="border-border bg-card/70 p-4">
          ...
          <div className="flex h-6 overflow-hidden rounded-md ring-1 ring-slate-200 dark:ring-white/10">
      ...
      {slowMovers.length > 0 ? (
        <Card className="border-orange-500/25 bg-orange-500/[0.05] p-4">
          <div className="mb-2 flex items-center gap-2">
            ...
            <span className="ml-auto text-sm font-bold tabular-nums text-orange-700 dark:text-orange-200">
              {fmt(summary.slow_movers_value || 0)} ₸ заморожено
            </span>
          </div>
          <div className="grid gap-1 md:grid-cols-2">
            {slowMovers.slice(0, 10).map((r) => (
              <div key={r.item_id} className="flex items-baseline justify-between gap-2 rounded-lg border border-orange-500/15 bg-orange-50 dark:bg-black/20 px-2.5 py-1.5 text-xs">
                <div className="min-w-0">
                  <p className="truncate text-orange-900/90 dark:text-orange-100/90">{r.name}</p>
                  <p className="text-[10px] text-orange-700/60 dark:text-orange-200/60">{r.category || 'Без категории'} · остаток: {r.stock_qty}</p>
                </div>
                <p className="tabular-nums font-semibold text-orange-700 dark:text-orange-200">{fmt(r.stock_value || 0)} ₸</p>
              </div>
            ))}
          </div>
          {slowMovers.length > 10 ? (
            <p className="mt-2 text-[11px] text-orange-700/70 dark:text-orange-200/70">…и ещё {slowMovers.length - 10}. Полный список — в основной таблице, фильтр «класс C» + сортировка по остатку.</p>
          ) : null}
        </Card>
      ) : null}
      ...
      {tab === 'sales' && summary.abc_xyz_matrix ? (
        <Card className="border-border bg-card/70 p-4">
          ...
          <div className="grid grid-cols-[80px_1fr_1fr_1fr] gap-1 text-xs">
            ...
            <div className="text-center font-semibold text-rose-700 dark:text-rose-300">Z · нестабильный</div>
          ...
          <div className="inline-flex rounded-lg border border-border bg-slate-100 dark:bg-white/[0.03] p-0.5 text-xs">
            {(['all', 'A', 'B', 'C'] as const).map((c) => (
              <button
                ...
                className={`rounded-md px-2.5 py-1 transition ${classFilter === c ? 'bg-white dark:bg-white/10 text-foreground' : 'text-muted-foreground hover:text-foreground'}`}
              >
                {c === 'all' ? 'Все' : c}
              </button>
            ))}
            ...
            <table className="w-full min-w-[780px] text-sm">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-white/[0.04]">
                {filteredRows.map((r) => {
                  ...
                  return (
                    <tr key={r.item_id} className="hover:bg-slate-50 dark:hover:bg-white/[0.02]">
                      ...
                      <td className="py-2 px-2 align-middle">
                        ...
                        {lowStock ? (
                          <p className="mt-0.5 inline-flex items-center gap-1 text-[10px] text-amber-700 dark:text-amber-300">
                      ...
                      <td className="py-2 px-2 text-right align-middle tabular-nums text-xs">
                        <span className={(r.margin_percent || 0) >= 30 ? 'text-emerald-700 dark:text-emerald-300' : (r.margin_percent || 0) >= 15 ? 'text-amber-700 dark:text-amber-300' : 'text-rose-700 dark:text-rose-300'}>
