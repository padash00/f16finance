...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      // Загружаем данные за текущий период И предыдущий для сравнения
      const { prevFrom } = calculatePrevPeriod(dateFrom, dateTo)
      const loadFrom = prevFrom // начало предыдущего периода
      const loadTo = dateTo     // конец текущего периода
  ...
  const analytics = useMemo(() => {
    ...
    // incomes - ВСЕ компании включая Extra
    ...
    // expenses - ВСЕ компании включая Extra
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="grid grid-cols-1 gap-4">
              ...
              <Card className="p-4 border border-border bg-card flex flex-col justify-center">
                ...
                <p className="text-[10px] text-muted-foreground mt-1">
                  На 1₸ затрат →{' '}
                  <span className="text-foreground">{insight.efficiency.toFixed(2)}₸</span> выручки
                </p>
              </Card>
            </div>
          </div>
                    ...
                    >
                      ...
                      <span
                        className={`text-xs font-bold font-mono ${
                          op.kind === 'income' ? 'text-green-400' : 'text-red-400'
                        }`}
                      >
                        {op.kind === 'income' ? '+' : '-'}
