...
import { AlertTriangle, CalendarDays, CheckCircle2, ChevronLeft, ChevronRight, Loader2, RefreshCw, Send } from 'lucide-react'
...
export default function OperatorSchedulePage() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`operator-schedule-${weekStart}`)
                ...
                <div className="space-y-5">
                  <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                    <div>
                      ...
                      <h2 className="mt-2 text-2xl font-semibold tracking-[-0.03em] text-white">{selectedBlock.company.name}</h2>
                            ...
                            <div key={shift.id} className="rounded-xl border border-white/8 bg-black/10 p-4">
                              <div className="flex items-start justify-between gap-3">
                                ...
                                {selectedBlock.requests.some(
                                  (item) => item.shift_date === shift.date && item.shift_type === shift.shift_type && item.status !== 'dismissed',
                                ) ? (
                              ...
                              {selectedBlock.response ? (
                                <div className="mt-4 space-y-3">
                                  ...
                                  <Button
                                    ...
                                    className="gap-2"
                                  >
                                    {submitting === issueKey ? <Loader2 className="h-4 w-4 animate-spin" /> : <AlertTriangle className="h-4 w-4" />}
                                    Сообщить о проблеме
                                  </Button>
                                </div>
                              ) : null}
